...
Scroller.prototype.setBounce = function(isBounce) {
	...
	if (isBounce === false) {
		...
		// 标记当前页使用了scroller (需延时,确保page已切换)
		setTimeout(function() {
			let uniPageDom = document.getElementsByTagName('uni-page')[0];
			uniPageDom && uniPageDom.setAttribute('use_scroller', true)
		}, 30);
		// 避免重复添加事件
		...
		window.bounceTouchmove = function(e) {
			...
			while (el !== document.body && el !== document) {
				if (el.tagName === 'UNI-PAGE') { // 只扫描当前页
					if (!el.getAttribute('use_scroller')) {
						isPrevent = false; // 如果当前页没有使用scroller,则不阻止
					}
					break;
				}
				let cls = el.classList;
				if (cls) {
					if (cls.contains('scroller-touch')) { // 采用scroll-view 此处不能过滤scroller,否则下拉仍然有回弹
						isPrevent = false; // scroller-touch无需拦截touchmove事件
						break;
					} else if (cls.contains('scroller-touch-x') || cls.contains('scroller-touch-y')) {
						// 如果配置了水平或者垂直滑动
						...
						if (z !== 0) {
							...
							if ((angle <= 45 && cls.contains('scroller-touch-x')) || (angle > 45 && cls.contains('scroller-touch-y'))) {
